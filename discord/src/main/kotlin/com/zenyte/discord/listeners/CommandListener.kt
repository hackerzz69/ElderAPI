...
class CommandListener : EventListener {
    ...
    override fun onEvent(event: GenericEvent) {
        ...
        val command = DiscordBot.getCommands().firstOrNull {
            val identifiers = it.identifiers.map { id -> id.lowercase() }
            identifiers.contains(identifier.lowercase())
        }
    ...
    private fun isCommand(message: Message): Boolean {
        if (!message.contentDisplay.startsWith(COMMAND_PREFIX)) return false
        return !message.author.isBot
    }

    private fun getCommandContents(message: Message) =
        message.contentDisplay.substring(COMMAND_PREFIX.length)
}
